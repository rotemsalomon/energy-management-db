...
def table2csv(db_file, table_name, csv_name):
    try:
       ...
       # Fetch column names in the table that match the filter
        ...
        filtered_columns = [col[1] for col in columns if col[1] in ['id', 'response_time', 'sensor_name', 'sensor_temperature', 'sensor_humidity']]

        # Ensure columns are ordered as per the predefined list
        desired_order = ['id', 'response_time', 'sensor_name', 'sensor_temperature', 'sensor_humidity']
...
if __name__ == '__main__':
    ...
    csv_name = '/root/projects/tasmota/scripts/raw_sensor_data_dump'
